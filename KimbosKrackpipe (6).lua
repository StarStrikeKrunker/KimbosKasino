...
local function SmoothFlyTo(position, speed)
    ...
    -- Anchor to prevent server from pulling us back
    local wasAnchored = HumanoidRootPart.Anchored
    HumanoidRootPart.Anchored = true
    
    -- Calculate waypoints: up -> across -> down
    local currentPos = HumanoidRootPart.Position
    local flightHeight = 200 -- Higher to clear obstacles
    
    -- More vertical path: go straight up, move across, come straight down
    local waypoints = {
        Vector3.new(currentPos.X, currentPos.Y + flightHeight, currentPos.Z), -- Go straight up
        Vector3.new(position.X, position.Y + flightHeight, position.Z),        -- Move across at height
        Vector3.new(position.X, position.Y + 10, position.Z),                  -- Come down (stop 10 studs above)
    ...
    for i, waypoint in ipairs(waypoints) do
        ...
        -- Set position again to ensure we're there
        if HumanoidRootPart then
            HumanoidRootPart.CFrame = CFrame.new(waypoint)
        end
    end
    
    -- Create invisible platform to prevent fall damage
    ...
    -- Unanchor when done and ensure final position
    wait(0.2)
    if HumanoidRootPart then
        HumanoidRootPart.CFrame = CFrame.new(position)
        HumanoidRootPart.Anchored = wasAnchored
        
        -- Force velocity to zero to prevent drift
        ...
        -- Wait a bit then remove platform
        wait(1)
        platform:Destroy()
    end
end
...
local function Escape()
    ...
    -- Calculate escape position 500 studs away
    local escapePos = HumanoidRootPart.Position + Vector3.new(500, 0, 500)
...
local function CreateGUI()
    ...
    StatusFrame.Size = UDim2.new(0, 330, 0, 140)
    ...
    CreateButton(HomePage, "💀 Respawn", UDim2.new(0, 15, 0, 190), UDim2.new(0, 160, 0, 32), Color3.fromRGB(192, 57, 43), function()
    ...
    CreateButton(HomePage, "🚨 Escape", UDim2.new(0, 210, 0, 190), UDim2.new(0, 160, 0, 32), Color3.fromRGB(230, 126, 34), function()
        Escape()
    end)
    ...
    aimbotLabel.Position = UDim2.new(0, 15, 0, 10)
    ...
    CreateToggle(AimbotPage, "Silent Aim", UDim2.new(0, 15, 0, 35), "SilentAim")
    CreateToggle(AimbotPage, "Triggerbot", UDim2.new(0, 15, 0, 80), "Triggerbot")
    CreateToggle(AimbotPage, "Show FOV", UDim2.new(0, 15, 0, 125), "ShowFOV")
    CreateToggle(AimbotPage, "Wall Check", UDim2.new(0, 15, 0, 170), "WallCheck")
    CreateToggle(AimbotPage, "Team Check", UDim2.new(0, 15, 0, 215), "TeamCheck")
    ...
    CreateToggle(CriminalsPage, "Auto Escape", UDim2.new(0, 15, 0, 15), "AutoEscape")
    ...
    CreateToggle(PolicePage, "Auto Arrest", UDim2.new(0, 15, 0, 15), "AutoArrest")
    ...
    CreateToggle(SettingsPage, "Anti-AFK", UDim2.new(0, 15, 0, 15), "AntiAFK")
    ...
    speedSlider.Position = UDim2.new(0, 15, 0, 90)
    ...
    CreateButton(SettingsPage, "💾 Save Config", UDim2.new(0, 15, 0, 140), UDim2.new(0, 160, 0, 32), Color3.fromRGB(39, 174, 96), function()
        SaveConfig()
    end)
    
    CreateButton(SettingsPage, "📁 Load Config", UDim2.new(0, 210, 0, 140), UDim2.new(0, 160, 0, 32), Color3.fromRGB(52, 152, 219), function()
        LoadConfig()
    end)
