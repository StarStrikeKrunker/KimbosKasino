...
local function FindStores()
    ...
    -- Search more thoroughly for stores
    for _, obj in pairs(Workspace:GetDescendants()) do
        local name = obj.Name
        
        -- Look for store models
        if (name == "DonutStore" or name == "GasStation" or name == "Store" or 
            name:match("Donut") or name:match("Gas") or name:match("Shop")) and 
            (obj:IsA("Model") or obj:IsA("Folder")) then
            table.insert(stores, obj)
        end
    end
    
    print("[KRACKPIPE] Found " .. #stores .. " stores")
    return stores
end
...
local function RobStore(store)
    ...
    UpdateStatus("Robbing " .. storeName)
    print("[KRACKPIPE] Attempting to rob: " .. storeName)
    
    -- Method 1: Find Till/Register
    local till = store:FindFirstChild("Till", true) or 
                 store:FindFirstChild("Register", true) or 
                 store:FindFirstChild("Counter", true)
    
    if till and till:IsA("BasePart") then
        TeleportTo(till.Position)
        wait(0.5)
        
        -- Try all descendants for prompts
        for _, obj in pairs(store:GetDescendants()) do
            if obj:IsA("ProximityPrompt") and obj.Enabled then
                pcall(function()
                    fireproximityprompt(obj)
                    print("[KRACKPIPE] ✓ Fired ProximityPrompt in " .. storeName)
        ...
        for _, obj in pairs(store:GetDescendants()) do
            if obj:IsA("ClickDetector") then
                pcall(function()
                    ...
                    print("[KRACKPIPE] ✓ Fired ClickDetector in " .. storeName)
                    Status.RobberiesCompleted = Status.RobberiesCompleted + 1
                    wait(3)
                end)
                return true
    ...
    -- Method 2: Just teleport to store center and look for prompts
    ...
    if storePart then
        ...
        for _, obj in pairs(store:GetDescendants()) do
            if obj:IsA("ProximityPrompt") and obj.Enabled then
                pcall(function()
                    fireproximityprompt(obj)
                    print("[KRACKPIPE] ✓ Robbed " .. storeName)
                    Status.RobberiesCompleted = Status.RobberiesCompleted + 1
                    wait(3)
                end)
                return true
    ...
    print("[KRACKPIPE] ✗ Could not rob " .. storeName)
    return false
end
...
local function CreateGUI()
    ...
    CreateToggle(CriminalPage, "Auto Rob", UDim2.new(0, 20, 0, 15), "AutoRob")
    CreateToggle(CriminalPage, "Auto Escape", UDim2.new(0, 20, 0, 60), "AutoEscape")
    
    local espLabel = Instance.new("TextLabel")
    espLabel.Parent = CriminalPage
    espLabel.BackgroundTransparency = 1
    espLabel.Position = UDim2.new(0, 20, 0, 110)
    espLabel.Size = UDim2.new(1, -40, 0, 20)
    espLabel.Font = Enum.Font.GothamBold
    espLabel.Text = "━━━━ CRIMINAL ESP ━━━━"
    espLabel.TextColor3 = Color3.fromRGB(255, 100, 100)
    espLabel.TextSize = 13
    
    local espToggle = CreateToggle(CriminalPage, "Enable ESP", UDim2.new(0, 20, 0, 135), "CriminalESP")
    espToggle.MouseButton1Click:Connect(function()
        wait(0.1)
        UpdateESP()
    end)
    
    CreateToggle(CriminalPage, "Names", UDim2.new(0, 20, 0, 180), "CriminalNames")
    CreateToggle(CriminalPage, "Boxes", UDim2.new(0, 20, 0, 225), "CriminalBoxes")
    CreateToggle(CriminalPage, "Distance", UDim2.new(0, 20, 0, 270), "CriminalDistance")
    ...
    policeEspLabel.Position = UDim2.new(0, 20, 0, 65)
    policeEspLabel.Size = UDim2.new(1, -40, 0, 20)
    ...
    local policeEspToggle = CreateToggle(PolicePage, "Enable ESP", UDim2.new(0, 20, 0, 90), "PoliceESP")
    policeEspToggle.MouseButton1Click:Connect(function()
        wait(0.1)
        UpdateESP()
    end)
    
    CreateToggle(PolicePage, "Names", UDim2.new(0, 20, 0, 135), "PoliceNames")
    CreateToggle(PolicePage, "Boxes", UDim2.new(0, 20, 0, 180), "PoliceBoxes")
    CreateToggle(PolicePage, "Distance", UDim2.new(0, 20, 0, 225), "PoliceDistance")
    ...
    CreateButton(SettingsPage, "💾 Save Config", UDim2.new(0, 20, 0, 65), UDim2.new(0, 170, 0, 35), Color3.fromRGB(39, 174, 96), function()
        SaveConfig()
    end)
    
    CreateButton(SettingsPage, "📁 Load Config", UDim2.new(0, 210, 0, 65), UDim2.new(0, 170, 0, 35), Color3.fromRGB(52, 152, 219), function()
        LoadConfig()
    end)
