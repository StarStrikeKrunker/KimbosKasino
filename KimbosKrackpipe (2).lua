...
local function SmoothFlyTo(position, speed)
    ...
    local flightHeight = 150
    
    local waypoints = {
        Vector3.new(currentPos.X, flightHeight, currentPos.Z), -- Go up
        Vector3.new(position.X, flightHeight, position.Z),      -- Move across at height
        position                                                 -- Come down to destination
    }
...
local function Escape()
    ...
    -- Smoothly fly to escape position at 75 studs/s
    SmoothFlyTo(escapePos, 75)
...
local function ArrestPlayer(criminal)
    ...
    -- Smoothly fly to criminal
    SmoothFlyTo(crimPos, 75)
...
-- Silent Aim using mouse input hook
...
mt.__namecall = newcclosure(function(self, ...)
    ...
    if Settings.SilentAim and tostring(self) == "Mouse" and (method == "Hit" or method == "Target") then
