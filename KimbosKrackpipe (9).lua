...
local function SmoothFlyTo(position, speed)
    ...
    -- Set to physics state to prevent fall damage
    ...
    -- Calculate waypoints: up -> across -> hover above ground
    local currentPos = HumanoidRootPart.Position
    local flightHeight = 200 -- Higher to clear obstacles
    
    -- Stop 50 studs above ground instead of going all the way down
    local safeHeight = 50
    ...
    local waypoints = {
        ...
        Vector3.new(position.X, position.Y + safeHeight, position.Z),          -- Hover 50 studs above ground
    }
    ...
    -- Now we're hovering at 50 studs - gently lower to ground
    wait(0.3)
    
    -- Slowly descend while still anchored (prevents fall damage)
    if HumanoidRootPart then
        for i = safeHeight, 5, -5 do
            if not HumanoidRootPart then break end
            local descendPos = Vector3.new(position.X, position.Y + i, position.Z)
            HumanoidRootPart.CFrame = CFrame.new(descendPos)
            wait(0.05)
        end
    ...
    if HumanoidRootPart then
        ...
        -- Re-enable falling after landing - stay in physics state longer
        wait(0.5)
        if humanoid then
