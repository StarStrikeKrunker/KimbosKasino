...
local function SmoothFlyTo(position, speed)
    ...
    -- ANCHOR immediately - this prevents ALL rubber-banding
    ...
    local waypoints = {
        ...
        Vector3.new(position.X, position.Y + 15, position.Z),
    }
    ...
    for i, waypoint in ipairs(waypoints) do
        ...
        while tick() - startTime < duration do
            ...
            local currentWaypoint = startPos:Lerp(waypoint, alpha)
            HumanoidRootPart.CFrame = CFrame.new(currentWaypoint)
    ...
    -- STAY ANCHORED for 2 seconds to ensure server accepts position
    wait(2)
    
    -- Final position confirmation
    HumanoidRootPart.CFrame = CFrame.new(Vector3.new(position.X, position.Y + 15, position.Z))
...
-- RayCast Hook (Magic Bullet) - Advanced Silent Aim
local oldRayCast = nil

local function SetupMagicBullet()
    pcall(function()
        if not oldRayCast then
            oldRayCast = require(game:GetService('ReplicatedStorage').Module.RayCast).RayIgnoreNonCollideWithIgnoreList
        end
        
        require(game:GetService('ReplicatedStorage').Module.RayCast).RayIgnoreNonCollideWithIgnoreList = function(...)
            local nearestDistance = Settings.FOVSize
                            ...
                            if onScreen then
                                ...
                                if screenDistance < nearestDistance then
                                    ...
                                    if Settings.WallCheck then
                                        ...
                                        if part and not v.Character:IsAncestorOf(part) then
                                            continue
                                        end
                                    end
...
-- Setup Magic Bullet on load
task.spawn(SetupMagicBullet)
...
local function ApplyGunMod()
    if not Settings.GunMod then return end
    
    pcall(function()
        for _, item in pairs(game:GetService("ReplicatedStorage").Game.ItemConfig:GetChildren()) do
            local config = require(item)
            if config then
                config.MagSize = math.huge
...
task.spawn(function()
    ...
    mouse.KeyDown:Connect(function(key)
        if key:lower() == "q" and Settings.InfNitro then
            pcall(function()
...
-- WalkSpeed/JumpPower Lock System
pcall(function()
    ...
    mt.__newindex = newcclosure(function(self, key, value)
        if self:IsA("Humanoid") then
            if key == "WalkSpeed" and Settings.LockWalkSpeed then
                return oldIndex(self, key, Settings.WalkSpeed)
            elseif key == "JumpPower" and Settings.LockJumpPower then
                return oldIndex(self, key, Settings.JumpPower)
            end
        end
        ...
        pcall(function()
            if Character then
                ...
                if humanoid and humanoid.SeatPart then
                    ...
                    -- Jailbreak vehicles use a specific hierarchy
                    local vehicle = seat.Parent
                    
                    if vehicle and vehicle:IsA("Model") then
                        -- Method 1: Direct VehicleSeat modification
                        if seat:IsA("VehicleSeat") then
                            seat.MaxSpeed = Settings.VehicleSpeed
                            seat.Torque = Settings.VehicleSpeed * 15
                            seat.TurnSpeed = 1
                        end
                        
                        -- Method 2: Find and modify Handling configuration (Jailbreak specific)
                        local handling = vehicle:FindFirstChild("Handling") or vehicle:FindFirstChild("Stats")
                        if handling then
                            for _, child in pairs(handling:GetChildren()) do
                                if child:IsA("NumberValue") then
                                    if child.Name == "Speed" or child.Name == "MaxSpeed" then
                                        child.Value = Settings.VehicleSpeed
                                    elseif child.Name == "Acceleration" then
                                        child.Value = Settings.VehicleSpeed / 2
                                    end
                                end
                            end
                        end
                        
                        -- Method 3: Modify all VehicleSeats in the model
                        for _, v in pairs(vehicle:GetDescendants()) do
                            if v:IsA("VehicleSeat") then
                                v.MaxSpeed = Settings.VehicleSpeed
                                v.Torque = Settings.VehicleSpeed * 15
                            end
                        end
                        
                        -- Method 4: Modify Engine (Jailbreak specific)
                        local engine = vehicle:FindFirstChild("Engine") or vehicle:FindFirstChild("Parts")
                        if engine then
                            for _, part in pairs(engine:GetChildren()) do
                                if part:IsA("NumberValue") and (part.Name:match("Speed") or part.Name:match("Power")) then
                                    part.Value = Settings.VehicleSpeed
                                end
                            end
                        end
                        
                        -- Method 5: BodyVelocity / BodyGyro modification
                        for _, obj in pairs(vehicle:GetDescendants()) do
                            if obj:IsA("BodyVelocity") then
                                obj.MaxForce = Vector3.new(math.huge, 0, math.huge)
                                -- Don't modify velocity directly, just max force
...
-- Bank Auto Rob (from reference script - WORKING)
local DuffelBag = game:GetService("MarketplaceService"):PlayerOwnsAsset(LocalPlayer, 2219040)
...
local function RobTheBank()
    for _, step in pairs(BankAutoRob) do
        if not Settings.AutoRob then break end
        
        UpdateStatus(step.Status)
        Notify("🏦 Auto Rob", step.Status)
        
        -- Teleport using double-set method for reliability
        for i=1,2 do
            wait()
            if HumanoidRootPart then
                HumanoidRootPart.CFrame = CFrame.new(unpack(step.Pos))
            end
        end
...
task.spawn(function()
    while wait(2) do
        pcall(function()
            ...
            if Bank then
                Bank = Bank:GetChildren()[1]
                if Bank then
                    local Info = Bank.Extra.Sign.Decal
                    
                    -- Check if bank is open
                    if Info.Transparency ~= 0 and Settings.AutoRob then
                        -- Check if we're close enough (within 150 studs)
                        local bankPos = Vector3.new(Info.Parent.Position.X, 0, Info.Parent.Position.Z)
                        local plrPos = Vector3.new(HumanoidRootPart.Position.X, 0, HumanoidRootPart.Position.Z)
                        
                        if (bankPos - plrPos).Magnitude < 150 then
                            RobTheBank()
                            wait(120) -- Wait 2 minutes before trying again
                        end
                    end
                end
            end
...
local function ShowKeySystem()
    local validKey = "AdminOp3n"
    local keyEntered = false
    
    -- Create key system GUI
    local KeyGui = Instance.new("ScreenGui")
    KeyGui.Name = "KrackpipeKeySystem"
    ...
    SubmitButton.MouseButton1Click:Connect(function()
        ...
        if enteredKey == validKey then
            ...
            for i = 1, 3 do
                KeyFrame.Position = UDim2.new(0.5, -200 + 10, 0.5, -150)
                wait(0.05)
                KeyFrame.Position = UDim2.new(0.5, -200 - 10, 0.5, -150)
                wait(0.05)
            end
...
local function ShowIntro()
    ...
    Subtitle.Font = Enum.Font.Gotham
    Subtitle.Text = "v1 BETA - Welcome to Agartha"
    Subtitle.TextColor3 = Color3.fromRGB(200, 200, 200)
    ...
    -- RGB animation for intro text
    task.spawn(function()
        while ScriptName and ScriptName.Parent do
    ...
    task.spawn(function()
        ...
        TweenService:Create(ScriptStroke, TweenInfo.new(0.8, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {Transparency = 0}):Play()
        wait(0.6)
        ...
        TweenService:Create(ScriptStroke, TweenInfo.new(0.7, Enum.EasingStyle.Quad, Enum.EasingDirection.In), {Transparency = 1}):Play()
...
local function CreateGUI()
    ...
    local hue = 0
    local function GetRGBColor()
        hue = hue + 0.0005  -- Much slower (was 0.003)
        if hue > 1 then hue = 0 end
        return Color3.fromHSV(hue, 0.8, 1)
    end
    ...
    TitleStroke.Color = Color3.fromRGB(100, 100, 110)  -- Static gray, no RGB
    ...
    aimbotLabel.Text = "━━━━ AIMBOT (Magic Bullet) ━━━━"
    aimbotLabel.TextColor3 = Color3.fromRGB(255, 100, 100)
    aimbotLabel.TextSize = 13
    
    local magicInfo = Instance.new("TextLabel")
    magicInfo.Parent = AimbotPage
    magicInfo.BackgroundTransparency = 1
    magicInfo.Position = UDim2.new(0, 330, 0, 35)
    magicInfo.Size = UDim2.new(0, 280, 0, 200)
    magicInfo.Font = Enum.Font.Gotham
    magicInfo.Text = "💫 Magic Bullet:\nBullets auto-redirect\nto nearest enemy\nin FOV circle.\n\nWorks with all guns\nincluding Taser!"
    magicInfo.TextColor3 = Color3.fromRGB(150, 150, 255)
    magicInfo.TextSize = 11
    magicInfo.TextYAlignment = Enum.TextYAlignment.Top
    magicInfo.TextXAlignment = Enum.TextXAlignment.Left
    magicInfo.TextWrapped = true
    ...
    CreateToggle(CriminalsPage, "Auto Rob Bank", UDim2.new(0, 20, 0, 53), "AutoRob")
    
    local gunModBtn = CreateToggle(CriminalsPage, "Gun Mod", UDim2.new(0, 20, 0, 91), "GunMod")
    gunModBtn.MouseButton1Click:Connect(function()
        wait(0.1)
        if Settings.GunMod then
            ApplyGunMod()
            Notify("🔫 Gun Mod", "Applied! Infinite ammo, auto, rapid fire")
        end
    end)
    
    CreateToggle(CriminalsPage, "Inf Nitro (Press Q)", UDim2.new(0, 20, 0, 129), "InfNitro")
    ...
    local function CreateSmallToggle(parent, text, position, settingKey)
        ...
        button.MouseButton1Click:Connect(function()
            ...
            -- Trigger ESP update when toggling
            UpdateESP()
        end)
    ...
    CreateSmallToggle(ESPPage, "Enable", UDim2.new(0, 10, 0, 30), "CriminalESP")
    ...
    CreateSmallToggle(ESPPage, "Enable", UDim2.new(0.5, 5, 0, 30), "PoliceESP")
    ...
    for i, tabName in ipairs(tabs) do
        ...
        tabButton.TextColor3 = Color3.fromRGB(0, 0, 0) or Color3.fromRGB(255, 255, 255)
...
print("[KRACKPIPE] Loading Kimbo's Krackpipe v1 BETA...")
...
-- Show key system
print("[KRACKPIPE] Awaiting key verification...")
local keyValid = ShowKeySystem()

if keyValid then
    print("[KRACKPIPE] ✓ Key verified! Loading GUI...")
    
    CreateGUI()
    
    Notify("✅ Ready", "Krackpipe active")
    print("[KRACKPIPE] ✅ Loaded successfully - v1 BETA")
    print("[KRACKPIPE] Welcome to Agartha - By Kimbo")
else
    print("[KRACKPIPE] ✗ Invalid key. Script terminated.")
end
