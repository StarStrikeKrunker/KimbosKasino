...
local function SmoothFlyTo(position, speed)
    ...
    -- Use PlatformStand to prevent falling and disable physics
    ...
    -- Stay 15 studs above ground at destination (never touch ground)
    local waypoints = {
    ...
    for i, waypoint in ipairs(waypoints) do
        ...
        -- Use CFrame instead of tweening for more control
        ...
        -- Ensure we're at the waypoint
        if HumanoidRootPart then
            HumanoidRootPart.CFrame = CFrame.new(waypoint)
        end
    end
    
    -- Keep player elevated with BodyPosition for anti-fall
    local bodyPos = Instance.new("BodyPosition")
    bodyPos.Position = Vector3.new(position.X, position.Y + 15, position.Z)
