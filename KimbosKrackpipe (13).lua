        ...
        pcall(function()
            if Character then
                ...
                if humanoid and humanoid.SeatPart then
                    ...
                    -- Jailbreak-specific: Find the vehicle model
                    local vehicle = seat:FindFirstAncestorOfClass("Model")
                    
                    if vehicle then
                        -- Method 1: Modify all VehicleSeats in the vehicle
                        for _, part in pairs(vehicle:GetDescendants()) do
                            if part:IsA("VehicleSeat") then
                                part.MaxSpeed = Settings.VehicleSpeed
                                part.Torque = Settings.VehicleSpeed * 10
                            end
                        end
                        
                        -- Method 2: Modify BodyVelocity/BodyGyro for Jailbreak vehicles
                        for _, part in pairs(vehicle:GetDescendants()) do
                            if part:IsA("BodyVelocity") then
                                part.MaxForce = Vector3.new(math.huge, math.huge, math.huge)
                                part.P = Settings.VehicleSpeed * 100
                            elseif part:IsA("BodyGyro") then
                                part.MaxTorque = Vector3.new(math.huge, math.huge, math.huge)
                            end
                        end
                        
                        -- Method 3: Jailbreak uses a 'Speed' or 'Engine' configuration
                        local config = vehicle:FindFirstChild("Configuration") or vehicle:FindFirstChild("Settings")
                        if config then
                            local speedValue = config:FindFirstChild("Speed") or config:FindFirstChild("MaxSpeed")
                            if speedValue and speedValue:IsA("NumberValue") then
                                speedValue.Value = Settings.VehicleSpeed
                            end
                        end
                        
                        -- Method 4: Directly modify Chassis or Engine if exists
                        local chassis = vehicle:FindFirstChild("Chassis") or vehicle:FindFirstChild("Engine")
                        if chassis then
                            for _, child in pairs(chassis:GetChildren()) do
                                if child:IsA("NumberValue") and (child.Name == "Speed" or child.Name == "MaxSpeed") then
